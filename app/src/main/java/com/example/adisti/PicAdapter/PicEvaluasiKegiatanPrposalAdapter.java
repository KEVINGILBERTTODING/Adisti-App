...
import com.example.adisti.PicFragment.PicVerifProposalFragment;
...
public class PicEvaluasiKegiatanPrposalAdapter extends RecyclerView.Adapter<PicEvaluasiKegiatanPrposalAdapter.ViewHolder> {
    Context context;
    List<ProposalModel>proposalModelList;
    PengajuInterface pengajuInterface;


    public PicEvaluasiKegiatanPrposalAdapter(Context context, List<ProposalModel> proposalModelList) {
    ...
    @NonNull
    @Override
    public PicEvaluasiKegiatanPrposalAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.list_proposal, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull PicEvaluasiKegiatanPrposalAdapter.ViewHolder holder, int position) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            cvMain.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Dialog dialogEvaluasiKegiatan = new Dialog(context);
                    dialogEvaluasiKegiatan.setContentView(R.layout.layout_evaluasi_kegiatan);
                    dialogEvaluasiKegiatan.getWindow().setBackgroundDrawableResource(android.R.color.transparent);
                    Button btnTutup = dialogEvaluasiKegiatan.findViewById(R.id.btnTutup);
                    EditText etP, etR, etS, etV;
                    etP = dialogEvaluasiKegiatan.findViewById(R.id.etP);
                    etR = dialogEvaluasiKegiatan.findViewById(R.id.etR);
                    etS = dialogEvaluasiKegiatan.findViewById(R.id.etS);
                    etV = dialogEvaluasiKegiatan.findViewById(R.id.etV);

                    etP.setText(proposalModelList.get(getAdapterPosition()).getP());
                    etR.setText(proposalModelList.get(getAdapterPosition()).getR());
                    etS.setText(proposalModelList.get(getAdapterPosition()).getS());
                    etV.setText(proposalModelList.get(getAdapterPosition()).getV());
                    btnTutup.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            dialogEvaluasiKegiatan.dismiss();
                        }
                    });
                    dialogEvaluasiKegiatan.show();
                }
            });
